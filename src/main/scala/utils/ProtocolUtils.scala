...
import models._
import models.transformer.types.SchemaParseException
      ...
      override def validateProtocol(protocol: Protocol, schema: IdlName): F[Protocol] =
        if (protocol.raw.trim.isEmpty)
          ...
          schema match {
            case IdlName.Avro =>
              Sync[F]
                .delay(parserAvro.parse(protocol.raw))
                .map(_ => protocol)
                .handleErrorWith(e =>
                  Sync[F].raiseError(
            ...
            case _ => Sync[F].raiseError(SchemaParseException(s"$schema type not implemented yet"))
          }
    }

  def apply[F[_]: ProtocolUtils]: ProtocolUtils[F] = ProtocolUtils[F]
}
